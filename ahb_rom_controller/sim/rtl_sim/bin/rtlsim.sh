...
if [ "${VERILOG_SIMULATOR:-iverilog}" = iverilog ]; then
    ...
    # Extra compile-time defines (e.g. "-D ASYNC_RST_EN=0"); empty by default = no-op.
    if [ $NODUMP -eq 1 ]
      then
        iverilog -o simv -c $SUBMIT_F -D SEED=$3 -D NODUMP ${SIM_EXTRA_DEFINES}
      else
        iverilog -o simv -c $SUBMIT_F -D SEED=$3 ${SIM_EXTRA_DEFINES}
    fi
    ...
    # Extra compile-time defines; empty by default = no-op. For non-iverilog
    # simulators use the appropriate +define+ syntax in SIM_EXTRA_DEFINES.
    vargs="$vargs ${SIM_EXTRA_DEFINES}"
