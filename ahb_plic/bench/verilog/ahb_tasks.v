...
//                      against the ahb_plic DUT. The PLIC slave exposes
//                      hprot_i / hsmode_i ports for the optional privilege
//                      filter (PRIV_CHECK_EN parameter). This BFM
//                      drives hprot / hsmode from the `mode` argument per
//                      the aRVern AHB dialect:
//                        USER       : hprot[1]=0, hsmode=0
//                        SUPERVISOR : hprot[1]=1, hsmode=1
//                        MACHINE    : hprot[1]=1, hsmode=0
//                      When the DUT has PRIV_CHECK_EN=0, these signals
//                      are still driven but ignored by the IP.
//----------------------------------------------------------------------------
...
task ahb_write;
   ...
   input   [1:0] mode;          // Privilege mode (USER / SUPERVISOR / MACHINE)
   ...
   begin
      ...
      if (mode == USER) begin
         ...
         hprot  = 4'h2;
         hsmode = (mode == SUPERVISOR) ? 1'b1 : 1'b0;
      end
      ...
      hwrite  = 1'b0;
      hsize   = 3'b000;
      hprot   = 4'h2;                       // Back to privileged-M idle
      hsmode  = 1'b0;
      if (blocking==1) begin
         @(posedge free_clk);
