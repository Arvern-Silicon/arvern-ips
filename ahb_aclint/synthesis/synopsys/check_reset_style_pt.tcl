...
#
#   Reads a SYNTHESIZED ahb_aclint netlist and classifies every flop's reset as
...
#   hresetn_i AND the low-frequency domain reset resetn_lf_i). RESET_PORTS lists
#   both, and the reset-fanout membership test below unions the fanout of every
#   reset port so flops in either domain are recognised as async-reset.
#----------------------------------------------------------------------------
...
proc check_reset_style {} {
    ...
    set DESIGN_NAME "ahb_aclint"
    set RESET_PORTS {hresetn_i resetn_lf_i}
