...
| `ahb_aclint`          | RISC-V ACLINT (Advanced Core-Local INTerruptor) as a single AHB-Lite slave, consolidating the MSWI / MTIMER / SSWI banks. The MTIME counter runs in an always-on low-frequency clock domain (gray-sync read, level-toggle write CDC) so timer ticks advance for wake-from-WFI while the main oscillator is off. | [`ahb_aclint/doc/ahb_aclint.md`](ahb_aclint/doc/ahb_aclint.md) |
