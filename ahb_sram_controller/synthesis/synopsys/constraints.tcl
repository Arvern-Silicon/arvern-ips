...
group_path -name FEEDTHROUGH -from [remove_from_collection [all_inputs] [get_ports hclk_i]] -to [remove_from_collection [all_outputs] [get_ports sram_clk_o]]
...
set MEM_DIN_DLY     [expr ($CLOCK_PERIOD/100) * 70]
set MEM_WEN_DLY     [expr ($CLOCK_PERIOD/100) * 70]

set_input_delay  $MEM_DOUT_DLY               -max -clock "hclk"   [get_ports sram_dout_i]
set_input_delay  0                           -min -clock "hclk"   [get_ports sram_dout_i]

set_output_delay $MEM_ADDR_DLY    -add_delay -max -clock "hclk"   [get_ports sram_addr_o]
set_output_delay 0                           -min -clock "hclk"   [get_ports sram_addr_o]

set_output_delay $MEM_CEN_DLY     -add_delay -max -clock "hclk"   [get_ports sram_cen_o]
set_output_delay 0                           -min -clock "hclk"   [get_ports sram_cen_o]

set_output_delay $MEM_DIN_DLY     -add_delay -max -clock "hclk"   [get_ports sram_din_o]
set_output_delay 0                           -min -clock "hclk"   [get_ports sram_din_o]

set_output_delay $MEM_WEN_DLY     -add_delay -max -clock "hclk"   [get_ports sram_wen_o]
