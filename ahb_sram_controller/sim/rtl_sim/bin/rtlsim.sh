...
if [ $# -ne $EXPECTED_ARGS ]; then
  ...
  echo "Example  : rtlsim.sh ./stimulus.v            ../../../bench/verilog/submit.f  123"
...
# Extra build-time defines (e.g. "-D ASYNC_RST_EN=0") passed through verbatim to
# the compile command.  No-op when SIM_EXTRA_DEFINES is unset/empty.
EXTRA_DEFINES=${SIM_EXTRA_DEFINES:-}
...
if [ "${VERILOG_SIMULATOR:-iverilog}" = iverilog ]; then
    ...
    if [ $NODUMP -eq 1 ]
      then
        iverilog -o simv -c $SUBMIT_F -D SEED=$3 -D NODUMP $EXTRA_DEFINES
      else
        iverilog -o simv -c $SUBMIT_F -D SEED=$3 $EXTRA_DEFINES
    fi
    ...
    # Append any extra build-time defines verbatim (no-op when unset/empty).
    # NB: SIM_EXTRA_DEFINES uses iverilog "-D NAME=VAL" syntax; for +define+
    # style simulators the caller is expected to provide the appropriate form.
    vargs="$vargs $EXTRA_DEFINES"
