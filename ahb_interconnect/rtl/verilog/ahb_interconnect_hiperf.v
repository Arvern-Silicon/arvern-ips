...
//         / \__/ \          Module:    ahb_interconnect_hiperf
//        /   /\   \         --------------------------------------------
...
//----------------------------------------------------------------------------
// File Name          : ahb_interconnect_hiperf.v
// Module Description : High-performance AHB interconnect variant.
//
...
// the OR-reduce of the per-block enables across both the executable (X)
// and non-executable (NX) sub-fabrics.
//----------------------------------------------------------------------------
`default_nettype none

module  ahb_interconnect_hiperf (
...
// EXECUTABLE AHB SUBORDINATE INTERFACES
//=========================================
input  wire        [32*NR_S_X-1:0] s_x_hrdata_i;
input  wire           [NR_S_X-1:0] s_x_hreadyout_i;
input  wire           [NR_S_X-1:0] s_x_hresp_i;

output wire        [32*NR_S_X-1:0] s_x_haddr_o;
output wire  [HAUSER_W*NR_S_X-1:0] s_x_hauser_o;
output wire         [3*NR_S_X-1:0] s_x_hburst_o;
output wire         [4*NR_S_X-1:0] s_x_hmaster_o;
output wire           [NR_S_X-1:0] s_x_hmastlock_o;
output wire         [4*NR_S_X-1:0] s_x_hprot_o;
output wire           [NR_S_X-1:0] s_x_hready_o;
output wire           [NR_S_X-1:0] s_x_hsel_o;
output wire         [3*NR_S_X-1:0] s_x_hsize_o;
output wire         [2*NR_S_X-1:0] s_x_htrans_o;
output wire        [32*NR_S_X-1:0] s_x_hwdata_o;
output wire           [NR_S_X-1:0] s_x_hwrite_o;
...
generate
    ...
    if (NR_S_X < 1) begin : CHECK_NR_S_X
        initial $fatal(1, "ahb_interconnect_hiperf: NR_S_X (%0d) must be >= 1.", NR_S_X);
    end
    if (NR_S_NX < 1) begin : CHECK_NR_S_NX
        initial $fatal(1, "ahb_interconnect_hiperf: NR_S_NX (%0d) must be >= 1.", NR_S_NX);
    end
    if (HAUSER_W < 1) begin : CHECK_HAUSER_W
        initial $fatal(1, "ahb_interconnect_hiperf: HAUSER_W (%0d) must be >= 1.", HAUSER_W);
    end
...
wire           [2*NR_S_X-1:0] s_x_request;
wire           [2*NR_S_X-1:0] s_x_grant;

wire                          x_hclk_en;
wire             [NR_S_X-1:0] x_hclk_en_manager_mux;
...
//=============================================================================
// 7)  AHB MANAGER MULTIPLEXORS
//=============================================================================

genvar ii;
generate
    for (ii = 0; ii < NR_S_X; ii = ii + 1) begin : AHB_MANAGER_MUX_X

        ahb_manager_mux #(.NR_M(2), .HAUSER_W(HAUSER_W), .ARST_EN(ASYNC_RST_EN)) ahb_manager_mux_inst_x (

        // AHB CLOCK & RESET
            .hclk_i            ( hclk_i                                                                        ),
            .hresetn_i         ( hresetn_i                                                                     ),

            .hclk_en_o         ( x_hclk_en_manager_mux[ii]                                                     ),

        // AHB MANAGER INTERFACES
            .m_haddr_i         ( {nx_haddr_to_x[32*ii+:32],              x_haddr_to_x[32*ii+:32]             } ),
            .m_hauser_i        ( {nx_hauser_to_x[HAUSER_W*ii+:HAUSER_W], x_hauser_to_x[HAUSER_W*ii+:HAUSER_W]} ),
            .m_hburst_i        ( {nx_hburst_to_x[3*ii+:3],               x_hburst_to_x[3*ii+:3]              } ),
            .m_hmaster_i       ( {nx_hmaster_to_x[4*ii+:4],              x_hmaster_to_x[4*ii+:4]             } ),
            .m_hmastlock_i     ( {nx_hmastlock_to_x[ii],                 x_hmastlock_to_x[ii]                } ),
            .m_hprot_i         ( {nx_hprot_to_x[4*ii+:4],                x_hprot_to_x[4*ii+:4]               } ),
            .m_hready_i        ( {nx_hready_to_x[ii],                    x_hready_to_x[ii]                   } ),
            .m_hsel_i          ( {nx_hsel_to_x[ii],                      x_hsel_to_x[ii]                     } ),
            .m_hsize_i         ( {nx_hsize_to_x[3*ii+:3],                x_hsize_to_x[3*ii+:3]               } ),
            .m_htrans_i        ( {nx_htrans_to_x[2*ii+:2],               x_htrans_to_x[2*ii+:2]              } ),
            .m_hwdata_i        ( {nx_hwdata_to_x[32*ii+:32],             x_hwdata_to_x[32*ii+:32]            } ),
            .m_hwrite_i        ( {nx_hwrite_to_x[ii],                    x_hwrite_to_x[ii]                   } ),

            .m_hrdata_o        ( {nx_hrdata_from_x[32*ii+:32],           x_hrdata_from_x[32*ii+:32]          } ),
            .m_hreadyout_o     ( {nx_hreadyout_from_x[ii],               x_hreadyout_from_x[ii]              } ),
            .m_hresp_o         ( {nx_hresp_from_x[ii],                   x_hresp_from_x[ii]                  } ),

        // ARBITER & ADDRESS DECODER INTERFACES
            .m_grant_i         ( s_x_grant[2*ii+:2]                                                            ),
            .m_request_o       ( s_x_request[2*ii+:2]                                                          ),

        // MAIN AHB INTERFACE
            .hrdata_i          ( s_x_hrdata_i[32*ii+:32]                                                       ),
            .hreadyout_i       ( s_x_hreadyout_i[ii]                                                           ),
            .hresp_i           ( s_x_hresp_i[ii]                                                               ),

            .haddr_o           ( s_x_haddr_o[32*ii+:32]                                                        ),
            .hauser_o          ( s_x_hauser_o[HAUSER_W*ii+:HAUSER_W]                                           ),
            .hburst_o          ( s_x_hburst_o[3*ii+:3]                                                         ),
        ...
        // ARBITER & ADDRESS DECODER INTERFACES
            .request_i         ( s_x_request[2*ii+:2]                                                          ),
            .grant_o           ( s_x_grant[2*ii+:2]                                                            )
        );
...
endmodule // ahb_interconnect_hiperf
